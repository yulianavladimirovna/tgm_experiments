...
# make bidirectional edges only for snapshots only on train!
df_train_rev = df_train_ui.copy()
df_train_rev[["from", "to"]] = df_train_rev[["to", "from"]]
df_train_mp = pd.concat([df_train_ui, df_train_rev], ignore_index=True).drop_duplicates(
    subset=["from", "to", "timestamp"]
)
df_train_mp = df_train_mp.sort_values("timestamp").reset_index(drop=True)

edge_index = torch.tensor(
    df_train_mp[["from", "to"]].to_numpy().T,
    device=device,
    dtype=torch.long,
).contiguous()
...
opt = torch.optim.Adam(decoder.parameters(), lr=float(args.lr))
...
with torch.no_grad():
    ...
    z = encoder(static_node_feats, edge_index).detach()

for epoch in range(1, int(args.epochs) + 1):
    with hm.activate(train_key):
        loss = train(train_loader, z, decoder, opt)
