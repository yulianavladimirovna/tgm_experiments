...
def eval_metrics_full_ranking(
    ...
    for batch in tqdm(loader):
        ...
        for i in range(int(batch.edge_dst.numel())): # by count dsts
            u = int(batch.edge_src[i].item())
            pos_gid = int(batch.edge_dst[i].item())                    # global item id
...
train_mp.static_node_feats = full_data.static_node_x
val_mp.static_node_feats = full_data.static_node_x
test_mp.static_node_feats = full_data.static_node_x
...
with torch.no_grad():
    encoder.eval()
    static_node_feats = train_dg.static_node_x
