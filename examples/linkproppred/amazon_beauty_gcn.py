...
def _sample_negs_exclusive(
    ...
    pos_dst = pos_dst.to(torch.int64)
    b = pos_dst.numel() # batch_size

    neg_local = torch.randint(
        0, # low
        int(num_items), # high
        (int(b), int(num_negs)), # size
    ...
    while mask.any():
        ...
        neg[mask] = neg_local2 + int(item_offset)
        mask = neg.eq(pos)
...
print(
    ...
    "t[min,max]=", int(df["timestamp"].min()), int(df["timestamp"].max()),
)
